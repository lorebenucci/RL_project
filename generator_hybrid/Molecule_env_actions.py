...
class MoleculeEnv:
    ...
    def _get_toxicity(self, mol):
        ...
        self.gnn_model.eval()
        with torch.no_grad():
            # --- MODIFICA: Scompatta logits e embedding ---
            logits,embedding = self.gnn_model(x, edge_index,edge_attr,batch, global_features=global_feats)
            probs = torch.sigmoid(logits)[0].cpu().numpy()
            
        is_toxic = np.any(probs > 0.5)
        return probs, is_toxic,embedding.cpu()
    ...
    def _get_state_from_embedding(self, embedding):
        # Converte il tensore embedding in un vettore numpy per l'Agente RL
    ...
    def _get_state(self):
        # Fallback se serve chiamarlo manualmente senza embedding pre-calcolato
    ...
    def step(self, action):
        ...
        try:
            ...
            tanimoto_sim = DataStructs.TanimotoSimilarity(fp_start, fp_current)
        except: tanimoto_sim = 0.0
        
        # 2. Embedding & Tossicità Attuale
        current_probs, current_is_toxic, current_embedding = self._get_toxicity(self.current_mol)
        
        
        cosine_sim = 0.0
        if self.start_embedding is not None and current_embedding is not None:
            # Cosine sim ritorna valore tra -1 e 1
            cosine_sim = torch.nn.functional.cosine_similarity(self.start_embedding, current_embedding).item()
        
        hybrid_sim = 0.5 * tanimoto_sim + 0.5 * cosine_sim
        
        if  hybrid_sim < self.threshold-0.2:
        ...
        delta_sim=hybrid_sim - self.previous_sim
        self.previous_sim=hybrid_sim
        ...
        if  hybrid_sim < self.threshold:
            ...
            if delta_sim > 0:
                reward += delta_sim * 15.0  # "Bravo, stai tornando verso la struttura originale"
            else:
                reward -= abs(delta_sim) * 15.0
        ...
        if current_probs is None: return self._get_state_from_embedding(current_embedding), -1.0, True, {'valid': False}
        ...
        next_state = self._get_state_from_embedding(current_embedding)
        info = {
            'valid': True, 
            'smiles': current_smiles, 
            'probs': current_probs, 
            'sim': hybrid_sim,  # IMPORTANTE: Passiamo la sim ibrida per l'oversampling
            'tanimoto': tanimoto_sim, # Per debug
            'cosine': cosine_sim,     # Per debug
            'flipped': has_flipped,
