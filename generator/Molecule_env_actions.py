...
class MoleculeEnv:
    def __init__(self, gnn_model,threshold=0.6, max_steps=5, device='cpu'):
    ...
    def step(self, action):
        ...
        if current_rings < self.start_rings:
            return self._get_state(), -10.0, True, {'valid': True, 'smiles': current_smiles,'flipped': False}
        ...
        if delta >0:
            reward += (delta * 10.0) * sim #scale for similarity
        else:
            reward -= 2.0 #penality if we go in opposite direction
        ...
        if has_flipped:
            reward += 50.0 * sim
            done=True # agent has flipped rightly the molecule we stop here
        else:
            reward += 2.0 * sim 
